      ...
      <Card>
        <CardContent className="space-y-4 p-6">
          ...
          <div className="flex flex-wrap gap-2">
            <Badge className="bg-primary-soft text-primary"><BadgeCheck className="size-3" /> {SERVICE_LABELS[offer.type_service]}</Badge>
            {offer.salaire != null && <Badge className="bg-secondary text-foreground">{formatFcfa(offer.salaire)}/mois</Badge>}
            {offer.logee && <Badge className="bg-secondary text-foreground"><Home className="size-3" /> Logée</Badge>}
          </div>
      ...
      {employer && (
        <Card className="mt-5">
          <CardContent className="flex items-center gap-3 p-4">
            ...
            <div>
              <p className="text-sm font-semibold">{`${employer.prenom ?? ""} ${employer.nom ?? ""}`.trim() || "Famille"}</p>
      ...
      <div className="mt-5">
        {viewer ? (
          <Button asChild size="lg" className="w-full">
            <Link href={`/app/offres/${id}`}><Send className="size-5" /> Postuler à cette offre</Link>
          </Button>
        ) : (
          <Card className="border-primary/30 bg-primary-soft/30">
            <CardContent className="flex flex-col items-center gap-3 p-6 text-center">
              <p className="text-sm text-muted-foreground">Connectez-vous ou créez un compte pour postuler.</p>
              <div className="flex w-full flex-col gap-2 sm:flex-row sm:justify-center">
                <Button asChild size="lg"><Link href={`/connexion?redirect=/app/offres/${id}`}><LogIn className="size-5" /> Se connecter</Link></Button>
                <Button asChild size="lg" variant="secondary"><Link href={`/inscription?role=candidate&redirect=/app/offres/${id}`}>Créer un compte</Link></Button>
              </div>
            </CardContent>
          </Card>
        )}
