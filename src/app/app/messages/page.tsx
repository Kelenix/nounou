...
export default async function MessagesPage() {
  ...
  return (
    <div className="space-y-4">
      <h1 className="text-2xl font-extrabold tracking-tight">{t("messages.title")}</h1>
      ...
      {conversations.length === 0 ? (
        <Card>
          <CardContent className="flex flex-col items-center gap-3 p-10 text-center">
            ...
            <h2 className="font-bold">{t("messages.noConversation")}</h2>
            <p className="max-w-xs text-sm text-muted-foreground">
              {profile.role === "employer" ? t("messages.emptyEmployer") : t("messages.emptyCandidate")}
            </p>
          </CardContent>
        </Card>
      ) : (
        <div className="divide-y divide-border overflow-hidden rounded-2xl border border-border bg-card">
          {conversations.map((c) => {
            const name = `${c.other.prenom ?? ""} ${c.other.nom ?? ""}`.trim() || t("messages.defaultUser");
            return (
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <span className="shrink-0 text-xs text-muted-foreground">
                      {new Date(c.lastAt).toLocaleDateString(dl, { day: "numeric", month: "short" })}
                    </span>
                  </div>
                  <p className="truncate text-sm text-muted-foreground">{c.lastMessage ?? t("messages.newConversation")}</p>
                </div>
                {c.unread > 0 && (
